...
> **문서 상태(2026-08-26):** Phase 11–16의 설계 이력이다. 사용자가 문서를 직접 붙여넣고
> `Target 이해 모델`을 별도로 만드는 여정은 파일럿 기본 흐름에서 더 이상 쓰지 않는다.
> 등록된 Target의 허용 origin 안에서만 OpenAPI를 읽는 좁은 예외와, 후보부터 실행까지의
> 단일 흐름은 [`DESIGN4.md`](DESIGN4.md)가 우선한다. 임의 URL·외부 `$ref`·문서 안 URL의
> 자동 fetch 금지와 모든 안전 경계는 그대로 유지한다.
